...
import {
  ...
  AgentContext,
  AgentResult,
} from "../agents/definitions/agent.interface";
...
export class PipelinesProcessor extends WorkerHost {
  ...
  async process(job: Job<CreatePipelineDto>): Promise<any> {
    ...
    this.logger.log(
      `[${jobId}] New job received. Starting pipeline for requestId: ${jobData.requestId}`,
    );
    ...
    try {
      ...
      // --- Saving Results ---
      this.logger.log(
        `[${jobId}] All agents executed. Saving final results to database...`,
      );
      ...
      this.logger.error(
        `[${jobId}] Pipeline failed: ${error.message}`,
        error.stack,
      );
  ...
  private async executeAndValidateAgent(
    ...
    for (let attempt = 1; attempt <= maxRetries && !isValid; attempt++) {
      ...
      if (!isValid) {
        this.logger.warn(
          `[${jobId}] ${agentName} validation failed. Critique: ${validationResult.metadata!.critique}. Retrying...`,
        );
      } else {
    ...
    if (!isValid) {
      this.logger.warn(
        `[${jobId}] ${agentName} failed validation after ${maxRetries} attempts. Proceeding with the last result.`,
      );
    }
